...
if (`"`subc'"'=="install" &`"`from'"'==""){
	...
	local pth=subinstr(`"`reps'"',`"`usr'"',"",1)
		...
		mata: files=select(files,strpos(files,`"<span class='simplified-path'>"'):=0)
		...
		if(`"`spkg'"'!=""){
		   di _n
		   di "tring to install packages: `spkg'"
		   di  _n
...

{
			//flag=strpos(filenames,".pkg")
			   ...
			   //flag
				if(sum(flag)>0){
				  st_global("pkgsinweb",strconcat(select(filenames,flag:>0)))
				  stata(`" di as red "Note: the specified repository includes $pkgsinweb " "')
				}
...
*** download the source code of the speicified website
	...
	cap mkdir _gitee_temp_
	local dirfolder `c(pwd)'/_gitee_temp_
	 
	!curl `"`website'"' -o  `"`dirfolder'/temp.txt"'
	 
	cap findfile  `"`dirfolder'/temp.txt"'
